...
c1.metric(
    t("Current Price (SAR/m²)", "السعر الحالي"),
    int(area_data["Avg_Price"].values[0])
)

c2.metric(
    t("AI Predicted Price", "السعر المتوقع"),
    int(predicted_price)
)
...
# =====================================
# AI Chat (Rule-based but smart)
# =====================================
st.markdown("## 💬 " + t("AI Assistant", "مساعد ذكي"))

question = st.text_input(
    t("Ask about investment decision...", "اسألي عن قرار الاستثمار...")
)

if question:
    st.success(t(
        f"Based on demand and risk, the recommendation is: {recommendation}",
